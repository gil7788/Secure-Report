...
double *my_vand1 (int degree, std::vector<int>& elements) {
    double *a;
    int i;
    int j;

    auto number_of_elements = elements.size();

    a = new double[number_of_elements * number_of_elements];
    for ( i = 0; i < number_of_elements; i++ ) {
        for ( j = 0; j < degree; j++ ) {
            if ( i == 0 && elements[j] == 0.0 ) {
                a[i+j*number_of_elements] = 1.0;
            }
            else {
                a[i+j*number_of_elements] = pow ( elements[j], i );
            }
        }
    }
