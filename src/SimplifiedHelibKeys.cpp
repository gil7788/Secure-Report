...
void SimplifiedHelibKeys::initKeys(long s, long R, long p, long r, long d, long c, long k, long w, long L, long chosen_m, const Vec<long> &gens, const Vec<long> &ords) {
//  long R=1;
//  long p=2;
//  long r=1;
//  long d=1;
...
Ctxt SimplifiedHelibKeys::encrypt(int b) {
    Ctxt r(*_publicKey);
    SimplifiedHelibKeys::encrypt(r, b);
    return r;
}

void SimplifiedHelibKeys::encrypt(Ctxt &r, int b) {
    std::vector<long> _b(nslots(), b);
    _ea->encrypt(r, *_publicKey, _b);
}

long SimplifiedHelibKeys::decrypt(const Ctxt &b) {
    std::vector<long> out;
    _ea->decrypt(b, *_secretKey, out);
    return out[0];
}

void SimplifiedHelibKeys::decrypt(vector<long> &out, const Ctxt &b) {
    _ea->decrypt(b, *_secretKey, out);
}

Ctxt SimplifiedHelibKeys::encrypt(const std::vector<int> &b) {
...
Ctxt SimplifiedHelibKeys::encrypt(const std::vector<long> &b) {
...
void SimplifiedHelibKeys::encrypt(Ctxt &r, const std::vector<int> &b) {
    std::vector<long> _b(b.size());
    for (unsigned int i = 0; i < b.size(); ++i)
        _b[i] = b[i];
    encrypt(r, _b);
...
void SimplifiedHelibKeys::encrypt(Ctxt &r, const std::vector<long> &b) {
    std::vector<long> _b(b);
    _b.resize(_ea->size(), 0);
    _ea->encrypt(r, *_publicKey, _b);
}

void SimplifiedHelibKeys::print(const Ctxt &b) {
...
void SimplifiedHelibKeys::write_to_file(std::ostream &out) {
    writeContextBase(out, *_context);
    out << *_context << endl;
    out << *_secretKey << endl;
}

void SimplifiedHelibKeys::read_from_file(std::istream &in) {
    unsigned long m;
