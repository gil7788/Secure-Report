...
public:
    SecureRetrievalTestCase(const string& name, int lookup_value,int database_size, int number_of_matches_in_database,
            ...
            _database_size{database_size}, _number_of_matches_in_database{number_of_matches_in_database},
...
public:
    SecureReportTestCase(const string& name, int lookup_value,int database_size, int number_of_matches_in_database,
                            DataType (*isMatch)(DataType&, DataType&), vector<int> plain_data,
                            unique_ptr<DatabaseDataType>& data_type_ptr): SecureRetrievalTestCase<DataType>(name, lookup_value, database_size, number_of_matches_in_database,
    ...
    unique_ptr<Data> test() override {
        ...
        unique_ptr<Data> protocol_data_ptr(new SecureRetrievalData<DataType>(this->_name, this->_lookup_value, this->_database_size, this->_number_of_matches_in_database,
...
public:
    SecureBatchRetrievalTestCase(const string& name, int lookup_value,int database_size, int number_of_matches_in_database, int requested_number_of_matches,
                            DataType (*isMatch)(DataType&, DataType&), vector<int> plain_data,
                            unique_ptr<DatabaseDataType>& data_type_ptr, int batch_size):
            _batch_size{batch_size}, SecureRetrievalTestCase<DataType>(name, lookup_value, database_size, number_of_matches_in_database,
                                          isMatch, plain_data, data_type_ptr) {}

    string to_string()  override{
        string str = SecureRetrievalTestCase<DataType>::to_string();
        str += "batch_size = " + std::to_string(get_batch_size());
        str += "number_of_matches_in_database = " + std::to_string(this->get_number_of_matches_in_database());
    ...
    unique_ptr<Data> test() override {
        ...
        auto start_retrieval_time = high_resolution_clock::now();
        retrieve_all_batches(protocol);
        auto end_retrieval_time = high_resolution_clock::now();
        auto retrieval_time = end_retrieval_time - start_retrieval_time;
        long retrieval_time_in_nanoseconds = duration_cast<nanoseconds>(retrieval_time).count();
         
         unique_ptr<Data> protocol_data_ptr(new SecureBatchRetrievalData<DataType>(this->_name, this->_lookup_value, this->_database_size, this->_number_of_matches_in_database,
